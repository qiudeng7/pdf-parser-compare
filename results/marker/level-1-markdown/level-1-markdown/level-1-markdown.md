...
- Docker Desktop 会在后台启动一个轻量级的 Linux 虚拟机(基于 WSL 2),名为 dockerdesktop 和 docker-desktop-data 。
...
- 存储在 WSL 发行版 docker-desktop-data 中,路径为 \\wsl\$\docker-desktopdata\version-pack-data\community\docker (通过 Windows 资源管理器访问)。
...
- Docker CLI 默认通过 Unix 套接字( /var/run/docker.sock )或 TCP 连接到 dockerdesktop 虚拟机中的守护进程,**无需手动配置**。
